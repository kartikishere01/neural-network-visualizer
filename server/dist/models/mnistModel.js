...
exports.getOrInitializeModel = getOrInitializeModel;
exports.runInference = runInference;
const tf = __importStar(require("@tensorflow/tfjs"));
// @ts-ignore
const mnist = __importStar(require("mnist"));
const path = __importStar(require("path"));
const fs = __importStar(require("fs"));
const MODEL_DIR = path.join(__dirname, '..', 'mnist-model-cache');
...
async function trainModel() {
    ...
    const xs = tf.tensor2d(set.training.map((d) => d.input));
    const ys = tf.tensor2d(set.training.map((d) => d.output));
    const valXs = tf.tensor2d(set.test.map((d) => d.input));
    const valYs = tf.tensor2d(set.test.map((d) => d.output));
    await m.fit(xs, ys, {
        epochs: 8,
        batchSize: 64,
    ...
    await m.save({
        save: async (artifacts) => {
            ...
            if (artifacts.weightData) {
                fs.writeFileSync(path.join(MODEL_DIR, 'weights.bin'), Buffer.from(new Uint8Array(artifacts.weightData)));
            }
...
async function getOrInitializeModel() {
    if (initPromise)
        return initPromise;
    initPromise = (async () => {
        if (model)
        ...
        if (model) {
            ...
            h1Model = tf.model({ inputs: inp, outputs: model.getLayer('hidden1').output });
            h2Model = tf.model({ inputs: inp, outputs: model.getLayer('hidden2').output });
            h3Model = tf.model({ inputs: inp, outputs: model.getLayer('hidden3').output });
            h4Model = tf.model({ inputs: inp, outputs: model.getLayer('hidden4').output });
        }
    })();
    return initPromise;
}
async function runInference(pixels) {
    ...
    return tf.tidy(() => {
        ...
        const h1Act = h1Model.predict(inputTensor);
        const h2Act = h2Model.predict(inputTensor);
        const h3Act = h3Model.predict(inputTensor);
        const h4Act = h4Model.predict(inputTensor);
        const outAct = model.predict(inputTensor);
